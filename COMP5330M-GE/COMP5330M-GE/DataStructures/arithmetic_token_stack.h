...
struct arithmetic_token_stack_32
{
	arithmetic_token states[32] = {};
	int top = 0;
	...
	inline
		arithmetic_token* top_state()
	{
		return this->states + this->top;
	}

	arithmetic_token read_state()
	{
		return *(this->top_state() - 1);
	}

	void push_state(arithmetic_token state)
	{
		*(this->top_state()) = state;
		this->top++;
	}

	arithmetic_token pop_state()
	{
		this->top--;
